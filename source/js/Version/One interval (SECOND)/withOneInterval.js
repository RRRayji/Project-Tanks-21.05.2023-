...
class Unit
{
	...
	shot()
	{
		if (!this.didShot)
		{
			bullet.push(new Bullet(this, unitSettings[(this.id < 2) ? this.id : 2].bulletModel, this.side));
			this.didShot = true;
	...
	setX(x_)
	{
		...
		{
			console.log(this);
			console.log(`player${this.id} center x,y: ${this.x + this.width / 2},${this.y + this.height / 2}`);
		}
	}
	setY(y_)
	{
...
class Player extends Unit
{
	...
	{
		super();
		this.model = unitSettings[this.id].model;
		gameZone.innerHTML += `<div id="player${this.id}"></div>`;
		this.element = document.querySelector(`#player${this.id}`);
		this.element.style.backgroundImage = this.model;
		this.initStyle();
		this.setX(x);
		this.setY(y);
		this.side = sides.top;
		this.controls = unitSettings[this.id].controls;
		this.isMoving = false;
		this.controller();
	}
	destruct()
	{
		unit.splice(this.id, 1);
		findInAndSplice(movableObject, this);
		gameZone.removeChild(this.element);
	}
	controller()
...
function isCollisionDetected(otherCoord, currUnit)
{
	...
	{
		switch (findUnit(currUnit.x, otherCoord.y, currUnit.id))
		{
	...
	{
		switch (findUnit(otherCoord.x, currUnit.y, currUnit.id))
		{
			case true: return true;
		}
		switch(currUnit.side)
